...
export default function StArbisUI() {
  ...
  const handleDeposit = React.useCallback(
    async ({ depositAmount }, { resetForm }) => {
      ...
      try {
        ...
        await transaction(userSigner.sendTransaction({ to: farmAddress, data } as any));
      } finally {
        setTimeout(() => {
          resetForm();
          populateState();
  ...
  const handleApproval = React.useCallback(async () => {
    if (!userSigner || !tokenContract || !state.tokenTotalSupply || !state.tokenAddress) {
      return;
    }
    ...
    try {
      ...
      transaction(userSigner.sendTransaction({ to: state.tokenAddress, data } as any));
    } finally {
      setTimeout(() => {
        populateState();
      }, 4000);
  ...
  return (
    <DashboardCard>
      ...
      <DashboardCard.Content>
        <p className="mt-4">
          Stake your ${state.tokenSymbol} for ${state.name} to earn passive ETH generated by fees
          from all around the Arbi's food court automatically!
        </p>
        ...
        {action === 'deposit' ? (
          <div className="mt-4">
            <Formik initialValues={{ depositAmount: '0' }} onSubmit={handleDeposit}>
                ...
                <Form method="post">
                  <fieldset disabled={isSubmitting}>
                    ...
                    <div className="mt-4">
                      ...
                      >
                        {state.isApproved ? (
                          <>{isSubmitting ? <span>staking...</span> : <span>stake</span>}</>
                        ) : (
                          <span>approve</span>
                        )}
                      </DashboardCard.Action>
        ...
        {action === 'withdraw' ? (
          <div className="mt-4">
            <Formik initialValues={{ withdrawAmount: '0' }} onSubmit={handleWithdraw}>
              {({ isSubmitting, handleSubmit, setFieldValue, values }) => (
                <Form method="post">
                  <div>
                    <span>MAX: </span>
                  ...
                  <div className="mt-4">
                    ...
                    >
                      {state.isApproved ? (
                        <>{isSubmitting ? <span>withdrawing...</span> : <span>withdraw</span>}</>
                      ) : (
                        <span>approve</span>
                      )}
                    </DashboardCard.Action>
